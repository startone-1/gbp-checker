...
一般的なアドバイスは一切禁止。この店舗固有の状況に基づいた、プロフェッショナルで具体的なアドバイスだけを出してください。

出力形式（各項目を長く、じっくり、細かく書いてください）：
1. 総合スコア: XX/100点 - 一言評価 + 詳細な評価理由
2. 規約違反チェック（危険度 + 該当ルール引用 + なぜ危険なのかの詳細説明）
3. 即修正できる具体的な改善案（この店舗に合わせた具体的な提案、コピペOKの文例を複数付きで長く）
4. 改善優先順位トップ5（各項目を詳しく、この店舗固有の理由を説明）
5. 先進施策（合法的なもののみ・この店舗に合わせた具体的な提案、実行方法、根拠を長く）
...
最後に必ず「これは参考情報です。最終判断はGoogle公式ツールで確認してください。」を入れてください。"""
        ...
        st.download_button("📄 診断結果をダウンロード", result, f"GBP詳細診断_{today}.html", "text/html")
