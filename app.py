...
<style>
    ...
    @media (max-width: 768px) {
        .tab-button { font-size: 1.25rem; padding: 20px 15px; }
    }
</style>
...
with col1:
    if st.button("🔗 Google Maps URLから診断", 
                 use_container_width=True, 
                 key="btn_gbp"):
        st.session_state.current_tab = "gbp"

with col2:
    if st.button("💬 レビュー返信アシスタント", 
                 use_container_width=True, 
                 key="btn_review"):
...
<div style="display:flex; gap:15px; margin-bottom:30px;">
    <div class="tab-button {'tab-button-active' if st.session_state.current_tab == 'gbp' else 'tab-button-inactive'}">🔗 Google Maps URLから診断</div>
    <div class="tab-button {'tab-button-active' if st.session_state.current_tab == 'review' else 'tab-button-inactive'}">💬 レビュー返信アシスタント</div>
</div>
""", unsafe_allow_html=True)
...
最後に免責事項を必ず入れてください。"""
            ...
            res = client.chat.completions.create(model="meta-llama/llama-4-maverick-17b-128e-instruct", messages=messages, max_tokens=3500, temperature=0.3)
